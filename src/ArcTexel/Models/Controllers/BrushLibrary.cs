...
internal class BrushLibrary
{
    ...
    public BrushLibrary(string pathToBrushes)
    {
        ...
        brushWatcher = new FileSystemWatcher(pathToBrushes, "*.arc");
    ...
    {
        Uri brushesUri = new Uri("avares://ArcTexel/Data/Brushes/");
        var assets = AssetLoader.GetAssets(brushesUri, null);

        foreach (var asset in assets)
        {
            string localPath = asset.LocalPath;
            if (localPath.EndsWith(".arc", StringComparison.OrdinalIgnoreCase))
            {
                try
                {
    ...
    private void LoadBrushesFromPath(string path)
    {
        ...
        var brushFiles = Directory.GetFiles(path, "*.arc", SearchOption.AllDirectories);
