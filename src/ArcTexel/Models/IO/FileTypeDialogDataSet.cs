...
internal class FileTypeDialogDataSet
{
    ...
    public FileTypeDialogDataSet(SetKind kind, IEnumerable<IoFileType> fileTypes = null)
    {
        ...
        else if (kind == SetKind.Arc)
        {
            Init(new LocalizedString("PIXI_FILE"), new[] { ArcFileType.ArcFile });
        }
        else if (kind == SetKind.Image)
        {
            Init(new LocalizedString("IMAGE_FILES"), allSupportedExtensions, ArcFileType.ArcFile);
        }
