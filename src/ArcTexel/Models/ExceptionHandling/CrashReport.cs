...
namespace ArcTexel.Models.ExceptionHandling;
...
internal class CrashReport : IDisposable
{
    public static CrashReport Generate(Exception exception, NonCrashInfo? nonCrashInfo)
    {
        ...
        builder
            .AppendLine(
                $"ArcTexel {VersionHelpers.GetCurrentAssemblyVersionString(moreSpecific: true)} x{IntPtr.Size * 8} crashed on {currentTime:yyyy.MM.dd} at {currentTime:HH:mm:ss} {currentTime:zzz}")
            .AppendLine(
    ...
    public int GetDocumentCount() => ZipFile.Entries.Where(x => x.FullName.EndsWith(".arc")).Count();

    public bool TryRecoverDocuments(out List<RecoveredArc> list, out CrashedSessionInfo? sessionInfo)
    {
        try
        {
    ...
    public List<RecoveredArc> RecoverDocuments(out CrashedSessionInfo? sessionInfo)
    {
        List<RecoveredArc> recoveredDocuments = new();
            ...
            recoveredDocuments.AddRange(
                ZipFile.Entries
                    .Where(x =>
                        ...
                        x.FullName.EndsWith(".arc"))
                    .Select(entry => new RecoveredArc(null, null, entry)));
        ...
        {
            recoveredDocuments.Add(new RecoveredArc(doc.OriginalPath, doc.AutosavePath,
    ...
    public void Save(IEnumerable<DocumentViewModel> documents)
    {
        ...
        foreach (var document in documents)
        {
            try
            {
                ...
                string nameInZip =
                    $"{fileName}-{document.OpenedUTC.ToString(CultureInfo.InvariantCulture)}-{counter.ToString(CultureInfo.InvariantCulture)}.arc"
                        .Replace(':', '_')
                        .Replace('/', '_');

                byte[] serialized = ArcParser.V5.Serialize(document.ToSerializable());
    ...
    public class RecoveredArc
    {
        ...
        public RecoveredArc(string? originalPath, string? autosavePath, ZipArchiveEntry recoveredEntry)
        {
        ...
        public byte[] TryGetAutoSaveBytes()
        {
            ...
            string autosaveArcFile = AutosavePath;
            if (!File.Exists(autosaveArcFile))
                return [];

            return File.ReadAllBytes(autosaveArcFile);
        }
    }
}
