...
namespace ArcTexel.Models.Commands.Search;
...
internal class FileSearchResult : SearchResult
{
    ...
    public override bool CanExecute => !asReferenceLayer ||
                CommandController.Current.Commands["ArcTexel.Clipboard.PasteReferenceLayerFromPath"].Methods.CanExecute(FilePath);
    ...
    public override void Execute()
    {
        if (!asReferenceLayer)
        {
            CommandController.Current.Commands["ArcTexel.File.OpenRecent"].Methods.Execute(FilePath);
        }
        else
        {
            var command = CommandController.Current.Commands["ArcTexel.Clipboard.PasteReferenceLayerFromPath"];
            if (command.Methods.CanExecute(FilePath))
            {
                CommandController.Current.Commands["ArcTexel.Clipboard.PasteReferenceLayerFromPath"].Methods
                    .Execute(FilePath);
            }
        }
    }
