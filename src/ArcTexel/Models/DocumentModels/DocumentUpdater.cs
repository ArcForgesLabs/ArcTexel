...
using ArcTexel.ChangeableDocument.Changeables.Graph.Exceptions;
using ArcTexel.ChangeableDocument.Changeables.Graph.Nodes;
using ArcTexel.ChangeableDocument.ChangeInfos;
using ArcTexel.ChangeableDocument.ChangeInfos.Animation;
using ArcTexel.ChangeableDocument.ChangeInfos.Drawing;
using ArcTexel.ChangeableDocument.ChangeInfos.NodeGraph;
using ArcTexel.ChangeableDocument.ChangeInfos.Properties;
using ArcTexel.ChangeableDocument.ChangeInfos.Root;
...
using ArcTexel.Models.DocumentPassthroughActions;
...
using ArcTexel.ChangeableDocument.ChangeInfos.NodeGraph.Blackboard;
...
internal class DocumentUpdater
{
    ...
    private void AddZoneIfNeeded(CreateNode_ChangeInfo info, NodeViewModel node)
    {
        if (node.Metadata?.PairNodeGuid != null)
        {
            ...
            if (otherNode != null)
            {
                ...
                if (!zoneExists)
                {
                    doc.NodeGraphHandler.AddZone(Guid.NewGuid(), $"ArcTexel.{info.Metadata.ZoneUniqueName}", node.Id,
    ...
    {
        if (info.InternalName != "ArcTexel." + BrushOutputNode.NodeId) return;
    ...
    {
        if (info.InternalName != "ArcTexel." + NestedDocumentNode.NodeId) return;
