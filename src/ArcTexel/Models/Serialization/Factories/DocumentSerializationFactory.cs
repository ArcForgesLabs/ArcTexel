...
{
    public override string DeserializationId { get; } = "ArcTexel.Document";
    ...
    public override byte[] Serialize(DocumentReference original)
    {
        ...
        int handle = Storage.AddFromBytes(original.ReferenceId.ToString(), ArcParser.V5.Serialize(vm.ToSerializable()));
        ...
        (string serializerName, string serializerVersion) serializerData)
    {
        if (serialized is byte[] bytes)
        {
            ...
            {
                return ArcParser.V5.Deserialize(toLoad).ToDocument();
            });
