...
[Command.Group("ArcTexel.File", "FILE")]
internal class FileViewModel : SubViewModel<ViewModelMain>
{
        ...
        : base(owner)
    {
        ...
        ArcTexelSettings.File.MaxOpenedRecently.ValueChanged += (_, value) => UpdateMaxRecentlyOpened(value);
    ...
    public void AddRecentlyOpened(string path)
    {
        ...
        int maxCount = ArcTexelSettings.File.MaxOpenedRecently.Value;
        ...
        ArcTexelSettings.File.RecentlyOpened.Value = RecentlyOpened.Select(x => x.FilePath);
    }

    [Command.Internal("ArcTexel.File.RemoveRecent")]
        ...
        else if (!args.Contains("--crash") && !args.Contains("--openedInExisting"))
        {
            if (preferences.GetLocalPreference("OnboardingV2Shown", false) == false)
            {
                ...
                if (IPlatform.Current?.IdentityProvider != null &&
                    IPlatform.Current.IdentityProvider.ProviderName == "ArcAuth")
                {
    ...
    [Command.Internal("ArcTexel.File.OpenRecent", AnalyticsTrack = true)]
    ...
    [Command.Basic("ArcTexel.File.Open", "OPEN", "OPEN_FILE", Key = Key.O, Modifiers = KeyModifiers.Control,
        MenuItemPath = "FILE/OPEN_FILE", MenuItemOrder = 1, Icon = ArcPerfectIcons.FileText, AnalyticsTrack = true)]
    ...
    [Command.Basic("ArcTexel.File.OpenFileFromClipboard", "OPEN_FILE_FROM_CLIPBOARD",
        "OPEN_FILE_FROM_CLIPBOARD_DESCRIPTIVE", CanExecute = "ArcTexel.Clipboard.CanPaste",
        Icon = ArcPerfectIcons.PasteAsNewLayer,
    ...
    [Command.Basic("ArcTexel.File.PlaceElement", "PLACE_ELEMENT", "PLACE_ELEMENT_DESCRIPTIVE",
        Key = Key.M, Modifiers = KeyModifiers.Control | KeyModifiers.Shift,
        MenuItemPath = "FILE/PLACE_ELEMENT", MenuItemOrder = 2, Icon = ArcPerfectIcons.FilePlus,
        AnalyticsTrack = true)]
    ...
    public DocumentViewModel OpenFromPath(string path, bool associatePath = true)
    {
        ...
        try
        {
            ...
            {
                return OpenDotArc(path, associatePath);
            }
    ...
    /// <summary>
    /// Opens a .arc file from path, creates a document from it, and adds it to the system
    /// </summary>
    private DocumentViewModel OpenDotArc(string path, bool associatePath = true)
    {
        ...
        Analytics.SendOpenFile(ArcFileType.ArcFile, fileSize, document.SizeBindable);
    ...
    /// Opens a .arc file from path, creates a document from it, and adds it to the system
    /// </summary>
    public void OpenRecoveredDotArc(string? originalPath, string? autosavePath, Guid? autosaveGuid,
        byte[] dotArcBytes)
    {
        DocumentViewModel document = Importer.ImportDocument(dotArcBytes, originalPath);
    ...
    public IDocument OpenFromArcBytes(byte[] bytes)
    {
    ...
    public void OpenFromReport(CrashReport report, out bool showMissingFilesDialog)
    {
        ...
        foreach (var document in documents)
        {
            ...
            try
            {
                OpenRecoveredDotArc(document.OriginalPath, document.AutosavePath,
    ...
    [Command.Basic("ArcTexel.File.New", "NEW_IMAGE", "CREATE_NEW_IMAGE", Key = Key.N,
        Modifiers = KeyModifiers.Control,
        MenuItemPath = "FILE/NEW_FILE", MenuItemOrder = 0, Icon = ArcPerfectIcons.File)]
    ...
    [Command.Basic("ArcTexel.File.Save", false, "SAVE", "SAVE_IMAGE", CanExecute = "ArcTexel.HasDocument",
        Key = Key.S, Modifiers = KeyModifiers.Control, Icon = ArcPerfectIcons.Save,
        MenuItemPath = "FILE/SAVE_PIXI", MenuItemOrder = 3, AnalyticsTrack = true)]
    [Command.Basic("ArcTexel.File.SaveAsNew", true, "SAVE_AS", "SAVE_IMAGE_AS", CanExecute = "ArcTexel.HasDocument",
        Key = Key.S, Modifiers = KeyModifiers.Control | KeyModifiers.Shift, Icon = ArcPerfectIcons.Save,
    ...
    [Command.Basic("ArcTexel.File.Export", "EXPORT", "EXPORT_IMAGE", CanExecute = "ArcTexel.HasDocument",
        Key = Key.E, Modifiers = KeyModifiers.Control,
        MenuItemPath = "FILE/EXPORT_IMG", MenuItemOrder = 5, Icon = ArcPerfectIcons.Image, AnalyticsTrack = true)]
    ...
    private void TryReopenTempAutosavedFiles()
    {
        ...
        // There are no autosave attempts .. but what if the user has just launched arctexel for the first time,
    ...
    {
        var paths = ArcTexelSettings.File.RecentlyOpened.Value.Take(ArcTexelSettings.File.MaxOpenedRecently
            .Value);
