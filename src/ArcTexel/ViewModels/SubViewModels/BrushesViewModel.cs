...
internal class BrushesViewModel : SubViewModel<ViewModelMain>
{
    ...
    [Command.Internal("ArcTexel.Brushes.Delete", "DELETE_BRUSH")]
    ...
    [Command.Basic("ArcTexel.Brushes.Import", "IMPORT_BRUSH", "IMPORT_BRUSH_DESCRIPTIVE")]
    public void ImportBrush()
    {
        var options = new FileTypeDialogDataSet(FileTypeDialogDataSet.SetKind.Arc).GetFormattedTypes(true);
    ...
    [Command.Internal("ArcTexel.Brushes.Edit", "EDIT_BRUSH")]
    ...
    [Command.Internal("ArcTexel.Brushes.Duplicate", "DUPLICATE_BRUSH")]
    public void DuplicateBrush(BrushViewModel brushViewModel)
    {
        ...
        else if(brushViewModel.Brush.Document is DocumentViewModel dvm)
        {
            string uniqueName = FileHelper.GetUniqueFileName(
                Path.Combine(Paths.PathToBrushesFolder, $"{brushViewModel.Brush.Name}_clone.arc"));
            Exporter.TrySave(dvm, uniqueName, new ExportConfig(dvm.SizeBindable), null);
        }
    }

    [Command.Basic("ArcTexel.Brushes.OpenBrushesFolder", "OPEN_BRUSHES_FOLDER", "OPEN_BRUSHES_FOLDER_DESCRIPTIVE",
