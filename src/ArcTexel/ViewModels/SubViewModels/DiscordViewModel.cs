...
internal class DiscordViewModel : SubViewModel<ViewModelMain>, IDisposable
{
        ...
        : base(owner)
    {
        ...
        Enabled = ArcTexelSettings.Discord.EnableRichPresence.Value;
        ArcTexelSettings.Discord.EnableRichPresence.ValueChanged += (_, value) => Enabled = value;
    ...
    public void UpdatePresence(DocumentViewModel? document)
    {
        ...
        if (document != null)
        {
            ...
            richPresence.Details = ArcTexelSettings.Discord.ShowDocumentName.Value
                ? $"Editing {document.FileName.Limit(128)}" : "Editing an image";

            string state = string.Empty;

            if (ArcTexelSettings.Discord.ShowDocumentSize.Value)
            {
                state = $"{document.Width}x{document.Height}";
            }

            if (ArcTexelSettings.Discord.ShowDocumentSize.Value && ArcTexelSettings.Discord.ShowLayerCount.Value)
            {
                state += ", ";
            }

            if (ArcTexelSettings.Discord.ShowLayerCount.Value)
            {
    ...
    private static RichPresence NewDefaultRP()
    {
        return new RichPresence
        {
            ...
            Assets = new Assets
            {
                ...
                LargeImageText = "You've discovered ArcTexel's logo",
                SmallImageKey = "github",
                SmallImageText = "Download ArcTexel (arctexel.net/download)!"
            },
