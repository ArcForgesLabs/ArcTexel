...
using ArcTexel.Models.Commands.Templates;
...
using ArcTexel.Views.Shortcuts;
...
internal partial class SettingsWindowViewModel : ViewModelBase
{
    ...
    [Command.Internal("ArcTexel.Shortcuts.Reset")]
    ...
    [Command.Internal("ArcTexel.Shortcuts.Export")]
    public static async Task ExportShortcuts()
    {
        ...
        var file = await MainWindow.Current!.StorageProvider.SaveFilePickerAsync(new()
        {
            ...
            {
                new FilePickerFileType("ArcShorts (*.arcsc)") { Patterns = new List<string> { "*.arcsc" }, },
    ...
    {
        errors = null;
        if (file.Name.EndsWith(".arcsc") || file.Name.EndsWith(".json"))
        {
            try
            {
    ...
    [Command.Internal("ArcTexel.Shortcuts.OpenTemplatePopup")]
