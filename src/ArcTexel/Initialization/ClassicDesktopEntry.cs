...
namespace ArcTexel.Initialization;
...
internal class ClassicDesktopEntry
{
    ...
    {
#if STEAM || DEV_STEAM
        return new ArcTexel.Platform.Steam.SteamPlatform();
#elif MSIX || MSIX_DEBUG
        return new ArcTexel.Platform.MSStore.MicrosoftStorePlatform(Paths.LocalExtensionPackagesPath, GetApiUrl(),
            GetApiKey());
#else
        return new ArcTexel.Platform.Standalone.StandalonePlatform(
    ...
    {
#if WINDOWS
        return new ArcTexel.Windows.WindowsOperatingSystem();
#elif LINUX
        return new ArcTexel.Linux.LinuxOperatingSystem();
#elif MACOS
        return new ArcTexel.MacOs.MacOperatingSystem();
#else
    ...
    {
        string? baseUrl = RuntimeConstants.ArcTexelApiUrl;
#if DEBUG
        if (baseUrl == null)
        {
    ...
    {
        string? apiKey = RuntimeConstants.ArcTexelApiKey;
#if DEBUG
        if (apiKey == null)
        {
