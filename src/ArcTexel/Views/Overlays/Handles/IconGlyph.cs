...
namespace ArcTexel.Views.Overlays.Handles;
...
public class IconGlyph : HandleGlyph
{
    ...
    private static Font? arcPerfectFont;

    public IconGlyph(string icon, Font font = null, Paint customPaint = null)
    {
        Icon = icon;
        arcPerfectFont ??= Font.FromStream(ArcPerfectIconExtensions.GetFontStream());
        targetFont = font ?? arcPerfectFont;
        if (customPaint != null)
        {
            fontPaint?.Dispose();
