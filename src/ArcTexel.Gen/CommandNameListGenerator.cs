...
{
    private const string Commands = "ArcTexel.Models.Commands.Attributes.Commands";

    private const string Evaluators = "ArcTexel.Models.Commands.Attributes.Evaluators";

    private const string Groups = "ArcTexel.Models.Commands.Attributes.Commands";

    private const string InternalNameAttribute = "ArcTexel.Models.Commands.Attributes.InternalNameAttribute";

    private static DiagnosticDescriptor commandDuplicate = new("Arc01", "Command/Evaluator duplicate", "{0} with name '{1}' is defined multiple times", "ArcTexel.Commands", DiagnosticSeverity.Error, true);
    ...
    private void AddSource(SourceProductionContext context, ImmutableArray<CommandMethod> methodNames, string name)
    {
        ...
        var nspace = SyntaxFactory
            .NamespaceDeclaration(SyntaxFactory.ParseName("ArcTexel.Models.Commands"))
            .AddMembers(cDecl);
