...
namespace ArcTexel.IdentityProvider.ArcAuth;

public class ArcAuthIdentityProvider : IIdentityProvider
{
    public string ProviderName { get; } = "ArcAuth";
    public bool AllowsLogout { get; } = true;
    public bool IsValid => isValid;
    public string InvalidInfo { get; } = "ACCOUNT_PROVIDER_NOT_AVAILABLE";
    private bool isValid = true;
    public ArcAuthClient ArcAuthClient { get; }
    public ArcUser User { get; private set; }
    ...
    public event Action<ArcUser>? LoginRequestSuccessful;
    ...
    public ArcAuthIdentityProvider(string arcEditorApiUrl, string? apiKey)
    {
        try
        {
            ArcAuthClient = new ArcAuthClient(arcEditorApiUrl, apiKey);
        }
    ...
    {
        User = SecureStorage.GetValue<ArcUser>("UserData", null);
        Task.Run(async () =>
        {
            await LoadUserData();
    ...
    public async Task RequestLogin(string email)
    {
        ...
        try
        {
            Guid? session = await ArcAuthClient.GenerateSession(email);
    ...
    public async Task ResendActivation(string email)
    {
        ...
        try
        {
            await ArcAuthClient.ResendActivation(User.SessionId.Value);
            LoginTimeout?.Invoke(60);
        }
    ...
    public async Task<bool> TryRefreshToken()
    {
        ...
        try
        {
            (string? token, DateTime? expirationDate) = await ArcAuthClient.RefreshToken(User.SessionToken);
    ...
    public async Task Logout()
    {
        ...
        try
        {
            await ArcAuthClient.Logout(sessionToken);
        }
        catch (ArcAuthException authException)
        {
        }
    ...
    public async Task LoadUserData()
    {
        try
        {
            ...
            var products = await ArcAuthClient.GetOwnedProducts(User.SessionToken);
            if (products != null)
            {
                User.OwnedProducts = products.Where(x => x is { IsDlc: true, Target: "ArcTexel" })
    ...
    public async Task<bool> TryValidateSession()
    {
        ...
        try
        {
            (string? token, DateTime? expirationDate) =
                await ArcAuthClient.TryClaimSessionToken(User.SessionId.Value);
            if (token != null)
            {
                User.SessionToken = token;
                User.SessionExpirationDate = expirationDate;
