...
using ArcTexel.Parser.Versions.DotArc4;
using ArcTexel.Parser.Versions.DotArc5;

namespace ArcTexel.Parser;

public abstract class ArcParser
{
    internal ArcParser() { }
    ...
    public static ArcParser<Document> V5 { get; } = new ArcParserArcV5();

    public static ArcParser<DocumentV4> V4 { get; } = new ArcParserArcV4();

    public static IArcDocument DeserializeUsingCompatible(Stream stream)
    {
        if (!TryGetCompatibleVersion(stream, out var parser))
        {
            throw new OldFileFormatException("This file was serialized pre .arc 4.0 and is not supported");
        }

        return parser.Deserialize(stream);
    }

    public static async Task<IArcDocument> DeserializeUsingCompatibleAsync(Stream stream, CancellationToken cancellationToken = default)
    {
    ...
    public static async Task<IArcParser> GetCompatibleVersionAsync(Stream stream, CancellationToken cancellationToken = default)
    {
        var header = new byte[HeaderLength];
    ...
    private static bool TryGetCompatibleFromVersion(Version version, [NotNullWhen(true)] out IArcParser parser)
    {
...
public abstract class ArcParser<T> : ArcParser, IArcParser where T : class, IArcDocument
{
    internal ArcParser() { }

    Task IArcParser.SerializeAsync(IArcDocument document, Stream stream, CancellationToken cancellationToken) =>
        SerializeAsync((T)document, stream, cancellationToken);

    void IArcParser.Serialize(IArcDocument document, Stream stream, CancellationToken cancellationToken) =>
        Serialize((T)document, stream, cancellationToken);

    IArcDocument IArcParser.Deserialize(Stream stream, CancellationToken cancellationToken) =>
        Deserialize(stream, cancellationToken);

    async Task<IArcDocument> IArcParser.DeserializeAsync(Stream stream, CancellationToken cancellationToken)
    {
