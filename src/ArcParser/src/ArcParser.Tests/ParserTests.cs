...
namespace ArcTexel.Parser.Tests;
...
public class ParserTests
{
    ...
    public void SerializingAndDeserializingWorks()
    {
        ...
        using FileStream stream = new("test.arc", FileMode.Create);
        ArcParser.V5.Serialize(document, stream);

        stream.Position = 0;

        var deserializedDocument = ArcParser.V5.Deserialize(stream);
    ...
    public async Task SerializingAndDeserialzingWorksAsync()
    {
        ...
        await using FileStream stream = new("testAsync.arc", FileMode.Create);
        await ArcParser.V5.SerializeAsync(document, stream);

        stream.Position = 0;

        var deserializedDocument = await ArcParser.V5.DeserializeAsync(stream);
    ...
    public void DetectCorruptedFile() =>
        Assert.Throws<InvalidFileException>(() => ArcParser.V5.Deserialize("./Files/CorruptedArcFile.arc"));
