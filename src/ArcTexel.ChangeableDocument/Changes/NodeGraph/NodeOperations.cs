...
public static class NodeOperations
{
    ...
    {
        if (nodeMap.TryGetValue(nodeUniqueName, out nodeType))
        {
            return true;
        }

        string normalizedName = NormalizeLegacyNodeUniqueName(nodeUniqueName);
        if (nodeMap.TryGetValue(normalizedName, out nodeType))
        {
            return true;
        }

        // Older resources serialized fully-qualified ids like "PixiEditor.Output".
        int dotIndex = normalizedName.LastIndexOf('.');
        if (dotIndex >= 0)
        {
            string shortName = normalizedName[(dotIndex + 1)..];
            if (nodeMap.TryGetValue(shortName, out nodeType))
            {
                return true;
            }
        }
    ...
    private static string NormalizeLegacyNodeUniqueName(string nodeUniqueName)
    {
        if (string.IsNullOrWhiteSpace(nodeUniqueName))
        {
            return string.Empty;
        }

        return nodeUniqueName
            .Replace("PixiEditor.", "ArcTexel.")
            .Replace("PixiParser.", "ArcTexel.Parser.")
            .Replace("PixiDocks.", "ArcDocks.");
    }
