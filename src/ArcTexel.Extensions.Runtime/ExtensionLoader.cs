...
using ArcTexel.Extensions.Metadata;
...
public class ExtensionLoader
{
    ...
    public void LoadExtensions()
    {
        foreach (var packagesPath in PackagesPath)
        {
            ...
            foreach (var updateFile in Directory.GetFiles(packagesPath, "*.update"))
            {
                try
                {
                    string newExtension = Path.ChangeExtension(updateFile, ".arcext");
            ...
            foreach (var file in Directory.GetFiles(packagesPath, "*.arcext"))
            {
                LoadExtension(file);
            }
        }
    ...
    // Uncomment when ArcTexel.Core extension concept is implemented
    /*private void LoadCore()
    {
        Type entry = typeof(ArcTexelCoreExtension);
        Assembly assembly = entry.Assembly;
        var serializer = new JsonSerializer();

        Uri uri = new Uri("avares://ArcTexel.Core/extension.json");
    ...
    private bool ValidateMetadata(ExtensionMetadata metadata, ExtensionEntry assembly)
    {
        ...
        if (fixedUniqueName.StartsWith("arctexel".Trim(), StringComparison.OrdinalIgnoreCase))
        {
